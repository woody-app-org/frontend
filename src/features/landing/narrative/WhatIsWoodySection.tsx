...
export function WhatIsWoodySection({ embedInLanding = false }: WhatIsWoodySectionProps) {
  ...
  const mobilePreview = whatIsWoody.mobilePreview;
  const hasMoreOnMobile = whatIsWoody.paragraphs.length > 1;
    ...
    <div ref={sectionRef} className="relative text-white">
      <div className="relative isolate min-h-[min(82svh,780px)] w-full overflow-hidden md:min-h-[min(88svh,860px)]">
        <img
          ...
          className={
            motion && !reduce
              ? "absolute inset-0 size-full object-cover object-[62%_38%] will-change-transform md:object-[center_20%]"
              : "absolute inset-0 size-full scale-[1.04] object-cover object-[62%_38%] md:object-[center_20%]"
          }
          style={
            motion && !reduce
        ...
        />
        <div
          className="absolute inset-0 bg-gradient-to-r from-black/68 via-black/38 to-black/10 md:from-black/75 md:via-black/35 md:to-transparent"
          aria-hidden
        />
        <div
          className="absolute inset-x-0 top-0 h-40 bg-gradient-to-b from-black/75 via-black/35 to-transparent md:h-36 md:from-black/55 md:via-transparent"
          aria-hidden
        />

        <div className="relative z-10 flex min-h-[min(82svh,780px)] flex-col justify-start px-[var(--layout-gutter)] pb-12 pt-28 md:min-h-[min(88svh,860px)] md:justify-center md:pb-24 md:pt-32">
                    ...
                    <>
                      ...
                      {hasMoreOnMobile ? (
                        <button
                          ...
                          className={cn(
                            "group mt-4 inline-flex items-center gap-2 rounded-full border border-white/25 bg-white/10 px-3.5 py-2 text-sm font-semibold text-white shadow-[0_2px_16px_rgba(0,0,0,0.35)] backdrop-blur-[2px] transition-[border-color,background-color,color]",
